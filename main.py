...
def login():
    if email_entry.get() == '' or password_entry1.get() == '':
        ...
        try:
            con = pymysql.connect(host='localhost', user='root', password='AshishIT45', database='bandtribe')
            mycursor = con.cursor()
...
def connect_database():
    ...
    mycursor.execute(query, (name_entry.get(), phone_entry.get(), email_entry1.get(), type_entry.get(),
                             specialist_entry.get(), password_entry.get()))
    con.commit()
    con.close()
...
# window.pack()
window.state('zoomed')
# window.geometry('1910x1070+50+50')
